...
Generates an output file that contains Tk values for k = [1-15]
"""
...
outfile.write('depth\ttotsites\tpropRA\tMLE_tk\n')
...
gatkData = [[] for k in range(max_kvalue)]
sitecounter = 0
for line in vcf:
        ...
        for j in indivRange:
            ...
            if annot != 'missing':
                ...
                if min_kvalue <= totDepth <= max_kvalue:
                    gatkData[totDepth - min_kvalue].append([phreds, q])
                else:
                    pass
            else:
